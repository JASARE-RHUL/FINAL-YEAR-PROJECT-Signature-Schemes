...
/**
 * Represents an item in the hash function selection list within the RSA key
 * generation view.
 * This class holds the name of the hash function and properties to manage
 * user selection
...
public class HashFunctionItem {
  ...
  /**
   * Property to store the custom hash size specified by the user when
   * "Custom" mode is selected.
   */
  private final StringProperty customHashSize;

  /**
   * Constructs a new HashFunctionItem with the specified name and
  ...
  /**
   * Gets the current selection mode for hash function output (e.g.,
   * "Standard", "Provably Secure").
   *
   * @return The selected hash function output mode.
   */
  public String getComboBoxSelection() {
    return comboBoxSelection.get();
  }
   ...
   */
  public String getCustomHashSize() {
    return customHashSize.get();
  }
   ...
   */
  public void setCustomHashSize(String size) {
