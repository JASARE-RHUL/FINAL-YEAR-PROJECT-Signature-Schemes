...
/**
 * This abstract class serves as the foundation for key generation
 * controllers within the
 * application. It defines the basic structure and operations for key
 * generation views while
 * allowing specific subclasses to implement the actual behavior for
...
public abstract class AbstractGenController {
   ...
   * interface and interactions necessary for the user to perform key
   * generation with the ability to
  ...
  /**
   * Loads the key generation view from the specified FXML file and sets up
   * the view and model
   * components. This method abstracts the common steps involved in loading
   * and configuring
   * different variations of the GenView.
   *
   * @param fxmlPath      The path to the FXML file that contains the layout
   *                      for the GenView.
   * @param observerSetup A Runnable containing additional setup steps for
   *                      event handlers and
   *                      observers, specific to the particular GenView being
   *                      loaded.
   */
  ...
  /**
   * Observer for changes in the application mode (Standard/Benchmarking).
   * This class listens to
   * changes in the application mode toggle switch and updates the GenView
   * accordingly. It switches
   * between the standard and benchmarking modes of the GenView based on the
   * user's selection.
   */
