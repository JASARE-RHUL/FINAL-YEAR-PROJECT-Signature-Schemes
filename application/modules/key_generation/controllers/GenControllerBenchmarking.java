...
/**
 * This class extends the AbstractGenController to implement functionality
 * specific to the
 * benchmarking mode of operation. It manages the interactions and data flow
 * for key generation
 * processes when benchmarking performance across different key parameters
 * and configurations is
 * required.
 */
public class GenControllerBenchmarking extends uk.msci.project.rsa.AbstractGenController {
   ...
   * benchmarking of key generation
   * processes. It provides methods to initiate benchmarking tasks, update
   * progress, and handle task
   * completion.
   */
  ...
  /**
   * Sets up observers for benchmarking related UI elements and actions in
   * the GenView. This method
   * configures event handlers and listeners for user interactions specific
   * to the benchmarking
   * process, such as comparison mode toggles, and cross-parameter
   * benchmarking mode changes. It is
   * used to enable the correct functioning of the GenView in different
  ...
  /**
   * Observer that handles the event when the number of keys button is
   * clicked. This observer is
   * responsible for initiating the process of entering multiple key sizes
   * (comparison mode (custom
   * vs (standard vs provably secure))) ar on a more granular level multiple
   * keys (non comparison
   * mode).
   */
  class NumKeysBtnObserver implements EventHandler<ActionEvent> {
    ...
    public void handle(ActionEvent event) {
      ...
      // Show the dynamic fields dialog and check if it was completed
      // successfully
      ...
      if (isFieldsDialogCompleted) {
        ...
        if (genView.showTrialsDialog(mainController.getPrimaryStage())) {
          ...
          uk.msci.project.rsa.BenchmarkingUtility.beginBenchmarkWithUtility(benchmarkingUtility, "Key Generation",
            benchmarkingTask,
            GenControllerBenchmarking.this::handleBenchmarkingCompletion,
            mainController.getPrimaryStage());
        }
      }
    }
  ...
  /**
   * Handles the completion of the benchmarking task for key generation. This
   * method is called when
   * the benchmarking task successfully completes. It initialises and sets up
   * the ResultsController
   * with the appropriate context (KeyGenerationContext) and displays the
  ...
  private void handleBenchmarkingCompletion() {
    uk.msci.project.rsa.ResultsControllerNormalBenchmarking resultsController = new ResultsControllerNormalBenchmarking(
      mainController);
    uk.msci.project.rsa.BenchmarkingContext context =
      new uk.msci.project.rsa.KeyGenerationContext(genModel);
  ...
  /**
   * Creates a background task for benchmarking key generation. This task
   ...
   * @param keyParams The parameters for key generation, including bit sizes
   *                  and the small e
   *                  option.
   * @return A Task to execute the benchmarking process in the background.
   */
  Task<Void> createBenchmarkingTask(int numTrials,
