...
public abstract class AbstractSignatureModelBenchmarking extends SignatureModel {
  ...
  /**
   * A list that stores the clock times for each trial during batch signature
   * generation. This is
  ...
  /**
   * A list to store the generated signatures from the benchmarking process.
  ...
  /**
   * A list to store the non-recoverable part of the messages from the
  ...
  /**
   * Stores the recoverable parts of messages from the verification process.
  ...
  /**
   * Stores the results of the verification process. Each boolean value in
  ...
  /**
   * A batch of keys used for generating or verifying signatures in a
   * benchmarking session.
   */
  List<Key> keyBatch = new ArrayList<>();

  /**
   * The number of trials to run in the benchmarking/batch methods. This
   * determines how many
  ...
  /**
   * An array storing the fraction used to determine the custom hash size
   * based on the key length.
   * The first element of the array represents the numerator, and the second
   * element represents the
   * denominator of the fraction. This array is used in the calculation of
   * custom hash sizes for
  ...
  /**
   * A list storing the lengths of the keys in the current key batch in bits.
  ...
  /**
   * The total number of signature operations completed during the current
   * benchmarking process.
   * This field tracks the progress of the batch signature creation and
  ...
  /**
   * The total amount of work for the comparison mode benchmarking. It
   * represents the total number
   * of signature operations that will be performed across all keys, hash
  ...
  /**
   * Sets the number of trials to be performed in the benchmarking of the
   * signature creation
   * process.
   *
  ...
  /**
   * Retrieves the number of trials set for the benchmarking of the signature
  ...
  /**
   * Adds a key to the batch of keys maintained by the model used for batch
   * operations in
   * benchmarking scenarios. Each key added can be utilised in subsequent
   * operations that involve
   * processing multiple keys in a single session.
   *
   * @param key The key to be added to the batch. This key should conform to
   *            the requirements of the
   *            operations for which the batch is intended (e.g., it should
   *            be a private key for
   *            signature generation or a public key for signature
   *            verification).
   */
  ...
  abstract void batchCreateSignatures(File batchMessageFile,
                                      DoubleConsumer progressUpdater)
    throws InvalidSignatureTypeException, NoSuchAlgorithmException,
    InvalidDigestException, NoSuchProviderException, IOException,
    DataFormatException, ExecutionException, InterruptedException;

  /**
   * Exports the batch of signatures generated during the benchmarking
   * process to a file. Each
   * signature is converted to a string representation and written as a
   * separate line in the file.
   *
   * @param signatureFileName The name of the file to which the signatures
   *                          will be exported.
   * @throws IOException If there is an error in creating the file or writing
   * to it.
   */
  public void exportSignatureBatch(String signatureFileName) throws IOException {
    ...
    try (BufferedWriter signatureWriter =
           new BufferedWriter(new FileWriter(signatureFile))) {
  ...
  /**
   * Exports the batch of non-recoverable message parts to a file. Each entry
   * consists of a flag (1
   * or 0) indicating whether a non-recoverable message part follows, and
   * then the message part
   * itself if present. Each entry is written on a new line.
   *
   * @param fileName The name of the file to which the non-recoverable
    ...
    try (BufferedWriter nonRecoverableWriter = new BufferedWriter(
      ...
      for (byte[] nonRecoverableMessage : nonRecoverableMessages) {
        if (nonRecoverableMessage.length > 0) {
          nonRecoverableWriter.write("1 ");  // Flag indicating a
          // non-recoverable message follows
          nonRecoverableWriter.write(new String(nonRecoverableMessage));
        } else {
          nonRecoverableWriter.write("0");  // Flag indicating no
  ...
  abstract void batchVerifySignatures(File batchMessageFile,
                                      File batchSignatureFile,
                                      DoubleConsumer progressUpdater)
    throws IOException, InvalidSignatureTypeException, DataFormatException,
    NoSuchAlgorithmException, InvalidDigestException, NoSuchProviderException;

  /**
   * Performs a single verification of a signature against corresponding
   * message using a given
   * signature scheme for utilisation during the batch verification process,
   * where multiple messages
   * and their respective signatures are verified. It handles different types
   ...
   * @param messageLine    The message (or non-recoverable part of the
   *                       message in schemes with
   *                       message recovery) to be verified against the
   *                       signature.
   * @param signatureBytes The byte array representing the signature to be
   *                       verified.
   * @return A Pair object where the first element is a Boolean indicating
   * the verification result
   * (true if verification succeeds, false otherwise), and the second element
   * is a Pair consisting
   * of the verification duration (in nanoseconds) and a List of byte arrays.
   * The List contains the
   * original message bytes, the signature bytes, and (if applicable) the
   * recovered message bytes.
   * @throws DataFormatException if the data format is incorrect or
  ...
  Pair<Boolean, Pair<Long, List<byte[]>>> getBatchVerificationResult(SigScheme sigScheme,
    ...
    if (getRecoveryStatus()) {
      ...
      byte[] nonRecoverableMessage =
        nonRecoverableParts[0].equals("1") ?
          nonRecoverableParts[1].getBytes() : null;
  ...
  /**
   * Verifies a signature against a message using a specified public key.
   * This method encapsulates
   * the logic for signature verification, handling different types of
   ...
   * @param digestSize     The size of the digest to be used in the
   *                       verification process.
   * @return A Pair containing the result of verification and the relevant
  ...
  Pair<Boolean, Pair<Long, List<byte[]>>> verifySignature(
    ...
    throws
    InvalidSignatureTypeException, DataFormatException,
    NoSuchAlgorithmException, InvalidDigestException, NoSuchProviderException {
   ...
   * benchmarking mode) or key
   * size index (comparison benchmarking). Each line in the file will contain
   * the index of the key
   * used for verification, the verification result, the original message,
   * the signature, and the
   * recovered message (if any).
   *
   * @param keyIndex        The index of the key/keySize for which
  ...
  /**
   * Retrieves the clock times recorded for each trial during the batch
   * signature creation process.
   * Each entry in the list represents the total time taken for a single trial.
   *
   * @return A list of long values, each representing the duration of a trial
   * in nanoseconds.
   */
  ...
  /**
   * Retrieves the list of signatures generated during the benchmarking
   * process. Each byte array in
  ...
  /**
   * Retrieves the list of non-recoverable message parts generated during the
   * signing process. These
   * parts are generated in signature schemes that involve message recovery.
   *
   * @return A list of byte arrays, where each array is a non-recoverable
   * part of a message.
   */
  ...
  /**
   * Retrieves the list of recoverable message parts generated during the
   * verification process.
   * These parts are recovered in signature schemes that support message
   * recovery.
   *
   * @return A list of byte arrays, where each array is a recoverable part of
   * a message.
   */
  ...
  /**
   * Computes the lengths of the keys in the provided batch in bits. This
   * method is used to
   * determine the size of each key, which is essential for various
   * cryptographic operations and
   * performance analysis. The length of each key is calculated based on the
   * modulus bit length,
   * rounded to the nearest byte size.
   *
   * @param keyBatch A list of keys whose lengths are to be calculated. These
   *                keys are typically
   *                 used in batch operations like signature generation or
   *                 verification.
   * @return A list of integers representing the length of each key in bits.
   * This list is parallel
   * to the input list, meaning each index in the return list corresponds to
   * the key at the same
  ...
  /**
   * Sets the list of key lengths based on the provided key batch. Each key's
   * length is calculated
   * and stored in the 'keyLengths' list.
   *
   * @param keyBatch A list of keys whose lengths are to be calculated and
   *                 stored.
   */
  ...
  /**
   * Retrieves the lengths of alls from the current key batch in bits. This
   * method provides insight
   * into the strength of the keys used in the signature process.
   *
   * @return A list of integer values, each representing the bit length of a
   * private key in the
   * batch.
   */
  ...
  /**
   * Sets the fraction used to calculate the custom hash size based on the
   * key length. The array
   * should contain two elements: the first element represents the numerator,
   * and the second element
   * represents the denominator of the fraction. This method is crucial for
   * configuring the model to
   * use custom hash sizes for signature operations when a variable length
   * hash function is
   * specified in benchmarking mode.
   *
   * @param customHashSizeFraction An array representing the fraction for
   *                               custom hash size
   *                               calculation.
   */
  ...
  /**
   * Aggregates the results from all verification trials into final lists.
   * This method combines the
   * times, verification results, signatures, and recovered messages from
   * each public key into
   * single lists for streamlined access and analysis.
   *
   * @param timesPerKey               The list of times for each public key
   *                                  and message.
   * @param verificationResultsPerKey The list of verification results for
   *                                  each public key and
   *                                  message.
   * @param signaturesPerKey          The list of signatures for each public
   *                                  key and message.
   * @param recoveredMessagesPerKey   The list of recovered message parts for
   *                                 each public key and
   *                                  message.
   */
  void combineVerificationResultsIntoFinalLists(List<List<Long>> timesPerKey,
    ...
    verificationResults =
      verificationResultsPerKey.stream().flatMap(List::stream)
    ...
    recoverableMessages = recoveredMessagesPerKey.stream().flatMap(List::stream)
      .collect(Collectors.toList());
    clockTimesPerTrial =
      timesPerKey.stream().flatMap(List::stream).collect(Collectors.toList());
  }
