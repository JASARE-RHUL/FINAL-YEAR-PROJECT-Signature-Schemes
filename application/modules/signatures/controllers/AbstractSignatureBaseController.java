...
/**
 * This abstract class is part of the controller component specific to
 * digital signature operations
 * responsible for handling user interactions for the signature process. It
 * also communicates with
 * the Signature Model to perform the actual signature processing logic. This
 * base controller
 * provides common functionalities used in both signature creation and
 * verification.
 */
public abstract class AbstractSignatureBaseController {

  /**
   * Holds batch of keys preloaded from cross benchmarking mode of the key
   * generation process as a
   * string. This string contains key data used for batch operations in
   * comparison mode. Each key in
  ...
  /**
   * Flag indicating whether the controller is operating in benchmarking mode
  ...
  /**
   * The main controller that orchestrates the flow between different views
   * of the application.
   */
  ...
  /**
   * Indicates whether the key currently being used is provably secure (small
   * e used to generate the
   * key).
   */
  ...
  /**
   * Indicates whether a single key that has been imported is provably secure
   * . This is relevant in
  ...
  /**
   * Sets up common observers for all signature views. This includes
   * observers for toggling
   * benchmarking mode, returning to the main menu, and other shared
   ...
   *                       view.
   */
  void setupCommonToAllObservers(Stage primaryStage,
  ...
  /**
   * Sets up observers specific to the standard signature operation mode
   * (non-cross-benchmarking).
   * These observers handle events related to signature scheme changes,
   * parameter choice changes,
  ...
  /**
   * Initializes observers for a signature view operating in standard mode
   * (non-benchmarking). This
   * includes observers for importing text and keys, handling cancel
  ...
  /**
   * Preloads a provably secure key into the signature view. This method is
   * used to set up the view
   * with a provably secure key, typically in a standard mode where a single
   * key is generated using
   * a small e is used for the signature process.
   *
   * @param signatureView The signature view to be updated with the preloaded
   *                      key.
   */
  ...
  /**
   * Processes a key file imported by the user. Validates the key format and
   * updates the signature
   * model and view based on the result of the validation.
   *
   * @param file           The key file selected by the user.
   * @param signatureView  The signature view to be updated based on the
   *                       imported key.
   * @param signatureModel The signature model to be updated with the
   *                       imported key.
   * @return true if the key is valid and imported successfully, false
   * otherwise.
   */
  ...
  class ImportObserver implements EventHandler<ActionEvent> {
     ...
     * @param fileConsumer   The TriConsumer that processes the selected file
  ...
  /**
   * The observer for exporting content to a file. This class handles the
   * action event triggered
   * when the user wants to export any kind of textual content. It is
   * responsible for facilitating
   * the file export process by using file handling utilities and displaying
   * relevant alerts to
  ...
  class ExportObserver implements EventHandler<ActionEvent> {
    ...
    /**
     * Constructs an ExportObserver with the specified file name, content,
     * and alert information.
     * This observer is responsible for exporting the provided content to a
     * file and displaying an
     * informational alert upon successful export.
     *
     * @param filename         The name of the file to which content is to be
     *                         exported.
     * @param fileContent      The content to be exported to the file.
     * @param infoAlertContent The message to be displayed in an alert upon
     *                         successful export.
     */
    public ExportObserver(String filename, String fileContent,
                          String infoAlertContent) {
  ...
  /**
   * The observer for copying content to the clipboard. This class handles
   * the action event
   * triggered when the user wants to copy any textual content to the
   * clipboard. It provides a
   * convenient interface for copying text and displays an error alert in
   * case of a failure.
   */
  class CopyToClipboardObserver implements EventHandler<ActionEvent> {
    ...
    /**
     * Constructs a CopyToClipboardObserver with specified content type,
     * content, and error alert
     * information. This observer is responsible for copying the provided
     * content to the clipboard
     * and displaying an error alert in case of a failure.
     *
     * @param contentType       The type of content to be copied (e.g.,
     *                          "signature").
     * @param content           The actual content to be copied to the
     *                          clipboard.
     * @param errorAlertContent The error message to be displayed if the copy
     *                          operation fails.
     */
    public CopyToClipboardObserver(String contentType, String content,
                                   String errorAlertContent) {
    ...
    public void handle(ActionEvent event) {
      try {
        uk.msci.project.rsa.DisplayUtility.copyToClipboard(content,
          contentType);
      } catch (Exception e) {
  ...
  /**
   * Handles the selection of a message file by the user. Reads the file
   * content and updates the
   * signature view to display the imported message. If the file content is
   * invalid or empty, an
   * error alert is shown.
   *
   * @param file           The file containing the message to be signed or
   *                       verified.
  ...
  /**
   * The observer for changes in signature scheme selection. This class
   * reacts to change in the
   * selected signature scheme and updates the model accordingly. It is
   * responsible for ensuring
   * that the signature model is aware of the currently selected signature
   * scheme, allowing for
  ...
  /**
   * Observer for changes in hash function selection. Reacts to change in the
   * selected hash function
   * and updates the model accordingly. Ensures that the signature model is
   * set with the correct
   * hash function based on the user selection and trigger the view to
   * display text field prompting
   * the user to enter a hash output size when they select a variable length
   * hash function in custom
   * mode.
   * <p>
   * Otherwise, preset options are used in the case of the standard hash
   * functions which are fixed
   * or the provably secure mode for variable length hash functions which
   * determines hash output
  ...
  /**
   * Configures the signature model to use a provably secure hash function
   * based on user selection
   * in the signature view. If the user chooses a variable length hash
   * function in custom mode,
   * prompts for the desired hash output size. Otherwise, sets the hash
   * function based on predefined
   * options for standard and provably secure modes.
   *
   * @param signatureView  The signature view providing the UI context for
   *                       hash function
   *                       configuration.
   * @param signatureModel The signature model to be configured with the hash
   *                       function.
   */
  ...
  /**
   * The observer for changes in parameter choice (standard vs. provably
   * secure vs. custom). This
   * class reacts to change in the selected parameter option and updates the
   * hash function drop down
   * menu immediately below with corresponding option.
   * <p>
   * For example in standard reveals on fixed hash function types while
   * provably secure and custom
   * mode reveal variable length hash function types with the distinction
   * being that customs lets
   * the user choose the output size in an adjacent text box that displayed
   * whereas provably secure
   * sets the output size of the variable length hash function to half the
   * length of the modulus for
   * the submitted key.
   */
  ...
  /**
   * The {@code ApplicationModeChangeObserver} class is an observer for
   * changes in application mode,
   * specifically for toggling between standard and benchmarking modes in
   * signature-related views.
   * This class observes for changes in a Boolean property (tied to a toggle
   * switch UI element) and
   * performs actions based on the mode switch. It uses two {@code Runnable}
   * objects to define the
   * actions to be executed when switching between standard and benchmarking
   * modes.
   * <p>
   * This observer provides flexibility and re-usability for different views
   * where such mode
  ...
  /**
   * Observer for canceling the import of a message in non benchmarking mode.
   * Handles the event when
   * the user decides to cancel the import of the message by replacing the
   * cancel button with the
   * original import button and resetting corresponding text field that
  ...
  /**
   * Sets the message to be signed or verified. This method is used to update
  ...
  /**
   * Observer for canceling the import of a key. Handles the event when the
  ...
  /**
   * Observer for changes in the selection of a radio button selection to
   * instantiate a scheme with provably secure parameters displayed only in
   * the event a provably secure key batch was preloaded.
   */
  ...
  /**
   * Updates the signature model and view with an imported key. This method
   * is used to update the
   * model with the key content and to update the view to reflect that a key
   ...
   * reflect that a key has been imported for instantiating a scheme with
   * provably secure in non
   * benchmarking mode. It updates the internal state with the imported key
   * batch.
   *
  ...
  public boolean handleHashOutputSize(SignatureBaseView signatureView) {
    try {
      if (Integer.parseInt(hashOutputSize) < 0
        ...
        uk.msci.project.rsa.DisplayUtility.showErrorAlert(
          "You must provide a non-negative integer for the hash output size. " +
            "Please try again.");
        return false;
      }
