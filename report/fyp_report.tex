...
\begin{enumerate}
    \item \textbf{Key Generation by Bob (Sender):}
    \begin{itemize}
        \item Bob runs \texttt{Gen}($1^\lambda$) to generate keys (pk, sk).
        \item He makes his public key (pk) available to everyone.
    \end{itemize}

    \item \textbf{Message Transmission:}
    \begin{itemize}
        \item To send a message \( m \) to Alice and ensure its authenticity, Bob generates a signature using his private key: \( \sigma \leftarrow \text{Sign}_{sk}(m) \).
        \item Bob sends both the message and its signature, (m, $\sigma$), to Alice.
    \end{itemize}

    \item \textbf{Verification by Alice (Receiver):}
    \begin{itemize}
        \item Upon receiving \( (m, \sigma) \) Alice verifies the authenticity of \( m \).
        \item She checks if \( \text{Vrfy}_{pk}(m, \sigma) \stackrel{?}{=} 1 \). This assures Alice that \( m \) was sent by Bob and wasn't modified in transit.
    \end{itemize}
\end{enumerate}
...
\begin{comment}
The idea revolves around pre-processing messages or in other words applying some transformation to a message m i.e., H(m) where H is a suitable (public) hash function, into elements of \( \mathbb{Z}^*_{N} \). 
\end{comment}
