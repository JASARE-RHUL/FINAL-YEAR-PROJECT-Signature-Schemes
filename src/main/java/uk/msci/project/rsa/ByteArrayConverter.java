...
public class ByteArrayConverter {
  ...
  /**
   * Converts a BigInteger to a byte array of a specified fixed length. This method ensures that the
   ...
   * @param emLen  The bit length based on which the size of the byte array is determined.
   * @return A byte array of length emBits/8 containing the two's-complement representation of the
   * BigInteger.
   * @throws IllegalArgumentException If the BigInteger's byte array is not of the expected length
  ...
  public static byte[] toFixedLengthByteArray(BigInteger number, int emLen)
    ...
    if (numberBytes.length == emLen) {
      ...
      System.arraycopy(numberBytes, 0, fixedLengthBytes, emLen - numberBytes.length,
          numberBytes.length);
      ...
      throw new IllegalArgumentException("The length of the byte array is incorrect");
    }
